  ...
  return (
    <nav className={twMerge('fixed left-1/2 -translate-x-1/2 bottom-6 z-50 backdrop-blur-sm bg-white/60 border-2 border-ink rounded-squircle-lg px-4 py-2 flex gap-3 items-center shadow-hard-stop', className)} aria-label="Navigation Dock">
      <div className="flex gap-3">
        <motion.button whileTap={{ scale: 0.98 }} transition={{ type: 'spring', stiffness: 1000, damping: 50 }} className="p-2 rounded-squircle-md border-2 border-ink bg-white shadow-hard-stop-sm">
        ...
        <motion.button whileTap={{ scale: 0.98 }} transition={{ type: 'spring', stiffness: 1000, damping: 50 }} className="p-2 rounded-squircle-md border-2 border-safety bg-white text-safety shadow-hard-stop-sm">
